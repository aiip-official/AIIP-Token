...
contract AIIP is ERC20Detailed {

  using SafeMath for uint256;
  mapping (address => uint256) private _balances;
  mapping (address => mapping (address => uint256)) private _allowed;

  string constant tokenName = "AIIP";
  string constant tokenSymbol = "AIIP";
...
*/
  ...
  function _burn(address account, uint256 amount) internal {
      uint256 bamount = amount.mul(1000000000);
    require(amount != 0);
    require(amount <= _balances[account]);
    _totalSupply = _totalSupply.sub(bamount);
    _balances[account] = _balances[account].sub(bamount);
